...
- 字幕直播间：可扫码直接加入，也可打开网址输入 6 位配对码，经主持人核对后安全加入。
...
live-caption/
├── public/                       # 无构建步骤的浏览器应用
│   ├── index.html                # 主持端页面（会话状态驱动的控制台）
│   ├── v1.html                   # 转正前的旧版面，仅作对照，见下方说明
│   ├── app.js                    # 会话、字幕、翻译、音频和 UI 编排
│   ├── v2.css                    # 桌面、手机、剧场与对话框样式
│   ├── styles.css                # 旧版面样式，随 v1.html 一同移除
...
2026-09-03 起 `index.html` 是新版面。旧版面留作 `v1.html` 对照，**不设固定删除日期**：
等新版面在真实会议里跑过、确认没有旧版才有的能力缺失，再连同 `styles.css` 一并删除。

留着不删的东西最后都会变成没人敢动的死代码，所以这不是一个可以无限期搁置的状态——
确认通过就删，别让它过夜太久。
...
主持端点击“扫码加入”创建临时房间。能扫码的参与者仍可直接加入；无法扫码时，可打开主持人展示的网址并输入 6 位配对码。参与页与主持端会显示相同的六位验证码，主持人核对并批准后，真实加入密钥才会通过临时 ECDH 密钥加密传给参与者。
