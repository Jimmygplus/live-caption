...
- 扫码字幕直播间：面向 non-speaking、Deaf 和 hard-of-hearing 参与者，支持字幕观看与文字发言。
...
| 模式 | 适合场景 | 字幕与翻译 | 多端参与 |
|---|---|---|---|
| 静态站点 | GitHub Pages、Cloudflare Pages、公开分享 | 用户在浏览器中配置自己的 Soniox / Anthropic 密钥 | 通过独立 Cloudflare relay 同步端到端加密数据 |
...
主持端点击“扫码加入”创建临时房间。参与端把文字发言放在字幕历史之前，最近字幕只提供上下文，完整记录可按需展开。
...
测试覆盖音频体检的安静语音、嘈杂房间、削波与静音样本，以及字幕 revision、布局、扫码加密房间、relay 状态和腾讯 TC3 签名。
